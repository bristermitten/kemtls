...
- We merge the McTiny Phase0 with the KEMTLS ClientHello phase
    ...
    - server responds with `ServerHello` containing the cookie C_0 _encrypted with the static shared secret_ (ss_s), plus its own `ServerRandom`/Nonce N
- then the McTiny flow continues as normal with 1 difference:
    - we use the KEMTLS KDF to derive keys from the ss_s instead of the McTiny constant shared secret S
        ...
        - **Details**:
            ...
            - then both parties can derive CHTS and STCS keys from ss_s - NOT ss_e like KEMTLS would
